...
const FetchCompany = () => {
  return async (dispatch) => {
    ...
    const FetchProfile = async () => {
      ...
      dispatch(
        company_action.setdegreelevel(response.data.education.degreelevel)
      );
      dispatch(company_action.setschool(response.data.education.school));
      dispatch(
        company_action.setsdescription(response.data.education.description)
      );
      ...
      dispatch(company_action.setportfolio(response.data.portfolio));
    };
    FetchProfile();
  };
};
